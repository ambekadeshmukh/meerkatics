#!/usr/bin/env python3
# sentinelops/scripts/test_metrics_collection.py
...
from sentinelops.metrics.token_counter import count_tokens
...
def test_metrics_collection(kafka_enabled=False):
    ...
    monitor = OpenAIMonitor(
        api_key=api_key,
        model="gpt-3.5-turbo",
        application_name="sentinelops-metrics-test",
        environment="testing",
        log_requests=True,
        log_responses=True,
...
def main():
    parser = argparse.ArgumentParser(description="Test SentinelOps metrics collection")
    parser.add_argument("--kafka", action="store_true", help="Enable Kafka integration")
    
    args = parser.parse_args()
    
    print("=== SentinelOps Metrics Collection Test ===")
