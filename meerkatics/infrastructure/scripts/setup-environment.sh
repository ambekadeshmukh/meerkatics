...
#!/bin/bash
# Setup script for creating a new Meerkatics environment

set -e

# Default values
ENV="development"
NAMESPACE="meerkatics-dev"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help)
      ...
      echo "  --namespace VALUE       Kubernetes namespace (default: meerkatics-dev)"
...
if [ "$NAMESPACE" == "meerkatics-dev" ] && [ "$ENV" != "development" ]; then
  NAMESPACE="meerkatics-$ENV"
fi

echo "Setting up Meerkatics $ENV environment in namespace $NAMESPACE"
...
cat > $VALUES_FILE << EOF
# Meerkatics values for $ENV environment
global:
  environment: $ENV
  imageTag: latest
...
# Install with Helm
echo "Deploying Meerkatics to $NAMESPACE namespace..."
...
echo "Meerkatics $ENV environment is ready in namespace $NAMESPACE"
echo "You can access the dashboard at: https://meerkatics-$ENV.your-domain.com"
